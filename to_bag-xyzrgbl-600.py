...
sem_value_1000_dict = dict(zip(sem_value_1000, sem_value_1000_index))    #{ 100:1, 50:2,  30:3, ...   }
...
r_sem,g_sem,b_sem = random2RGB(len(sem_value_1000))      # RGB_sem = [[0,200,0],...,[10,20,10]]         #这里的r_sem的颜色和语义的序号对应，如： [10,200,20]:1,  [20,100,4]:2, [40,20,100]:3, ...,
...
def depth2xyzl(depth_map,semantic_image,fx,fy,cx,cy,flatten=False,depth_scale=1000):     #xyzl : 实际为xyzrgb，rgb对应label
    h,w=np.mgrid[0:depth_map.shape[0],0:depth_map.shape[1]]
    z=depth_map/depth_scale
    x=(w-cx)*z/fx
    y=(h-cy)*z/fy
    ...
    xyzrgb=np.dstack((x,y,z,r,g,b)) if flatten==False else np.dstack((x,y,z,r,g,b)).reshape(-1,6)
    return xyzrgb

def depth2xyzli(depth_map,semantic_image,instance_image,fx,fy,cx,cy,flatten=False,depth_scale=1000):     #xyzli : 实际为xyzrgbi，rgb对应label，i为instance
    h,w=np.mgrid[0:depth_map.shape[0],0:depth_map.shape[1]]
    z=depth_map/depth_scale
    x=(w-cx)*z/fx
    y=(h-cy)*z/fy
    ...
    # new_im = ImagePIL.fromarray(b)     #调用Image库，数组归一化 
    # misc.imsave('new_img.jpg', new_im)
    xyzrgbl=np.dstack((x,y,z,r,g,b,I)) if flatten==False else np.dstack((x,y,z,r,g,b,I)).reshape(-1,7)
    return xyzrgbl
...
def CreateBag():#img,imu, bagname, timestamps
    ...
    '''read time stamps'''
    # 所转化场景的根目录
    root_dir = "/media/tang/Elements/dataset/scannet/scene0010_01/"
    # 返回所有图片的位置
    imgs_depth = ReadImages(root_dir+"frames/depth/")
    imgs_color = ReadRGBImages(root_dir+"frames/color/")
    imgs_instance = ReadImages(root_dir+"frames/instance-filt/")
    imgs_semantic = ReadImages(root_dir+"frames/semantic-filt/")
    ...
    # 所创建rosbag的目录和名称
    bag = rosbag.Bag("/media/tang/Elements/dataset/scannet/scene_0010_01.bag", 'w')
    ...
    try:    
        ...
        for i in range(len(imgs_depth)):
            ...
            resize_semantic = cv2.resize(raw_semantic,(640,480))
             #读入instance
            raw_instance = cv2.imread(imgs_instance[i], -1)
            resize_instance = cv2.resize(raw_instance,(640,480))
            ...
            pc = depth2xyzli(raw_depth_np,resize_semantic,resize_instance, 577.870605, 577.870605, 319.500000, 239.500000, flatten=True)
            header = Header()
            header.frame_id = "/camera"
            header.stamp = rospy.Time.from_sec(float(imagetimestamps[i]))
            fields = [PointField('x', 0, PointField.FLOAT32, 1),
                    ...
                    PointField('i', 24, PointField.FLOAT32, 1)]
